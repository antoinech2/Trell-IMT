...
def insert_initial_values(*args, **kwargs):
    ...
    db.session.add(Etiquette(type="priority", label="Low", description="Not important task", color="03ad31"))
    db.session.add(Etiquette(type="priority", label="Lowest", description="Lowest important task", color="4aa8c2"))
    db.session.add(
        Etiquette(type="status", label="Not started", description="Planned but not started yet", color="6d6f8a"))
    ...
    db.session.add(
        Etiquette(type="status", label="Waiting", description="Waiting for more information", color="d106ce"))
